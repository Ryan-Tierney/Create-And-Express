...
class CreationContainer extends React.Component {
  ...
  renderMyCreations = () => {
    return (
      <>
        <button
          className='btn btn--creation'
          onClick={this.openNewCreationForm}
        >
          Creation
        </button>
        <section className='cards'>
          {this.props.currentUser &&
            this.props.creations
              .filter(
                (creation) => creation.user.id === this.props.currentUser.id
              )
              .map((creation) => (
                <Creation
                  key={creation.id}
  ...
  searchedCreations = () => {
    switch (this.props.searchOption) {
      case "name":
        return this.publicCreations().filter((creation) =>
          creation.name.toLowerCase().includes(this.props.search.toLowerCase())
        );
      case "username":
        return this.publicCreations().filter((creation) =>
      ...
      case "like":
        return this.publicCreations().filter(
          (creation) => creation.likes === true
        );
      default:
  ...
  filteredCreations = () =>
    this.props.filter === "All"
      ? this.searchedCreations()
      : this.searchedCreations().filter(
          (creation) => creation.category.name === this.props.filter
        );

  sortedCreations = () => {
    switch (this.props.sort) {
      case "alphabetically":
        return this.filteredCreations().sort((a, b) =>
      ...
      case "alphabetically-by-city":
        return this.filteredCreations().sort((a, b) =>
          a.location.city.localeCompare(b.location.city)
        );
      case "most-popular":
        return this.filteredCreations().sort((a, b) => b.likes - a.likes);
      default:
        return this.publicCreations();
    }
  };
  ...
  renderAllCreations = () => {
    return (
      <>
        ...
        <section className='cards'>
          {this.props.creations &&
            this.sortedCreations().map((creation) => (
              <Creation key={creation.id} {...creation} all={true} />
            ))}
        </section>
      </>
    );
...
export default withRouter(
  connect(mapStateToProps, { getCreations, addCreation, editCreation })(
    CreationContainer
  )
);
